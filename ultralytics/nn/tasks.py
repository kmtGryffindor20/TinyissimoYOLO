...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(
        ...
        if m in (
            ...
            RepC3,
            RepConv,
        ) or m.__name__ in ("DSConv", "DSBlock"):
            c1, c2 = ch[f], args[0]
            if (
                c2 != nc
            ...
            if m in (
                ...
                n = 1
        elif m.__name__ in ("DSInception", "DSInceptionBlock"):
            c1 = ch[f]
            c2 = sum(args[:4])
            args = [c1, *args]
        elif m.__name__ == "Add":
            # Element-wise Add / residual-style module.
            # Output channels equal input channels (assume inputs have matching channels).
            if isinstance(f, list):
                c1 = ch[f[0]]
            else:
                c1 = ch[f]
            c2 = c1
            # Prepend input channel to args so the module receives channel info if required.
            # args = [c1, *args]
        elif m.__name__ in ("DSResBlock"):
            c1 = ch[f]
            args = [c1]
            c2 = c1
